...
/****** Object:  StoredProcedure [dbo].[taSopHdrIvcInsertPost]    Script Date: 19/12/2018 05:51:38 p. m. ******/
SET ANSI_NULLS ON
GO
...
/*	IF @I_vSOPTYPE = 4
	BEGIN
		IF NOT EXISTS(SELECT SOPNUMBE FROM INT_SOPHDR WHERE INTEGRATIONID = convert(int, @I_vINTEGRATIONID) AND SOPTYPE = 3)
		BEGIN
			select
				@iStatus = 0,
				@iAddCodeErrState = 0
...
/*	DECLARE @TOTAL NUMERIC(19,5), @IMPUESTO NUMERIC(19,5)
	...
	IF ABS(@TOTAL - @I_vDOCAMNT) > 10 OR ABS(@IMPUESTO - @I_vTAXAMNT) > 10
	BEGIN
		select
			@iStatus = 0,
			@iAddCodeErrState = 0
...
*/
	...
	DECLARE @APPLYTAX smallint, @APPLYWTH smallint, @TAXAMNT NUMERIC(19,5), @ORTAXAMT NUMERIC(19,5)
	...
	IF @TAXAMNT > 0 BEGIN SELECT @APPLYTAX = 1 END ELSE BEGIN SELECT @APPLYTAX = 0 END
	
	IF ABS(@WTHAMNT) > 0 BEGIN SELECT @APPLYWTH = 1 END ELSE BEGIN SELECT @APPLYWTH = 0 END
	...
	INSERT INTO nsatw_sop10100 
	SELECT @I_vSOPTYPE, @I_vSOPNUMBE, 'Sales Entry', @I_vBACHNUMB, @APPLYTAX, @TAXAMNT, @ORTAXAMT, @APPLYWTH, @WTHAMNT, @OWTHAMNT
