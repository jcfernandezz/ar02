...
/****** Object:  StoredProcedure [dbo].[taSopHdrIvcInsertPost]    Script Date: 19/12/2018 05:58:31 p. m. ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	BEGIN
        ...
        --MSAL 04-09-2018. CUando no integra por diferencia de importes se saca el control.
		--IF (ABS(@TOTAL - @I_vDOCAMNT) > 1 OR ABS(@IMPUESTO - @I_vTAXAMNT) > 1 ) AND 1>1
		...
		INSERT INTO INT_SOPHDR SELECT @I_vSOPTYPE, @I_vSOPNUMBE, @I_vINTEGRATIONID, @I_vDOCAMNT, @I_vTAXAMNT, (SELECT RTRIM(CONVERT(char(255), memo)) from INTDB2..ERP_Invoice WHERE invoice_id = CONVERT(int, @I_vINTEGRATIONID)), GETDATE()
	...
	BEGIN
		...
		IF @NOTEINDX <> 0
		BEGIN
			SELECT @NOTE = RTRIM(ADDRESS1) + CHAR(13) + RTRIM(ADDRESS2) + CHAR(13) + RTRIM(ADDRESS3) + CHAR(13) + 
					RTRIM(ZIP) + ' - ' + RTRIM(CITY) + CHAR(13) + RTRIM(STATE) + ' - ' + 
					ISNULL((SELECT RTRIM(CCodeDesc) FROM DYNAMICS..UPR41105 WHERE CCode = RM.COUNTRY), RTRIM(COUNTRY))
					FROM RM00101 RM where CUSTNMBR = @I_vCUSTNMBR

			DELETE SY03900 WHERE NOTEINDX = @NOTEINDX
	...
	BEGIN
		...
		BEGIN		
			...
			-- ES LA MODIFICACION PARA QUE NO INTEGREN TRX EN JULIO. EL OR=GINAL ES:
			-- if DAY(@I_vDOCDATE) > @DIATOPE
			if DAY(@I_vDOCDATE) >= @DIATOPE
			BEGIN	
				select
					@iStatus = 0,
					@iAddCodeErrState = 67004
