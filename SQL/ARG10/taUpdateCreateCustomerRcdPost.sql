...
/****** Object:  StoredProcedure [dbo].[taUpdateCreateCustomerRcdPost]    Script Date: 07/10/2018 18:09:22 ******/
SET ANSI_NULLS ON
GO
...
if NOT exists(SELECT invoice_status FROM INTDB2..ERP_Invoice WHERE invoice_id IN(
SELECT invoice_id FROM INTDB2..INT_INVOICE_VIEW
...
IF NOT EXISTS(SELECT CUSTNMBR FROM AWLI_RM00101 WHERE CUSTNMBR = @I_vCUSTNMBR)
BEGIN
	INSERT INTO AWLI_RM00101 SELECT CUSTNMBR, 1 OP_ORIGIN, 
		CASE USERDEF1	
			WHEN 'Consumidor Final'	THEN '05'
			WHEN 'Exento'			THEN '04'
			WHEN 'Exportaciones'	THEN '04'
			WHEN 'IVA Exento'		THEN '04'
			WHEN 'IVA Inscripto'	THEN '01'
			WHEN 'Monotributo'		THEN '06'
		ELSE	'' END RESP_TYPE, '' CUIT_Pais, 0 GrossIncomeStatus, '' GrossIncomeNumber
	FROM RM00101 WHERE CUSTNMBR = @I_vCUSTNMBR
END 
