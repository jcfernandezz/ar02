...
/****** Object:  StoredProcedure [dbo].[taSopHdrIvcInsertPost]    Script Date: 19/12/2018 05:21:50 p. m. ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	IF ABS(@TOTAL - @I_vDOCAMNT) > 100 OR ABS(@IMPUESTO - @I_vTAXAMNT) > 100
	BEGIN
		select
			@iStatus = 0,
			@iAddCodeErrState = 0
...
-- PARA CHILE PIDIERON QUITAR EL CONTROL DE FECHA TOPE  * * * * * * * * * * * * * * * * * * * * * * 
...
IF NOT EXISTS(SELECT LOCHTRXNO FROM LOCH0004 WHERE LOCHTRXNO = @I_vSOPNUMBE AND DOCTYPE = CASE WHEN @I_vSOPTYPE = 3 THEN 1 ELSE 8 END and module1 = 2)
	BEGIN
	INSERT INTO LOCH0004 
		SELECT @I_vSOPNUMBE LOCHTRXNO, CASE WHEN @I_vSOPTYPE = 3 THEN 1 ELSE 8 END DOCTYPE,
			CASE WHEN @I_vSOPTYPE = 3 THEN '33' ELSE '61' END LOCHDOCCOD, @I_vCUSTNMBR CUSTVNDR,
			'' VNDDOCNM, @I_vDOCDATE DOCDATE, 2 MODULE1, @I_vDOCAMNT DOCAMNT, 2 RPRTTYPE, 
			'sa' USERID, 1 DOCCLTYP
	END
else
	update LOCH0004 set custvndr = @I_vCUSTNMBR
	where LOCHTRXNO = @I_vSOPNUMBE 
	AND DOCTYPE = CASE WHEN @I_vSOPTYPE = 3 THEN 1 ELSE 8 END
	and module1 = 2		--2:ventas
