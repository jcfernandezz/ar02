...
/****** Object:  StoredProcedure [dbo].[taUpdateCreateCustomerRcdPost]    Script Date: 19/12/2018 05:28:21 p. m. ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [dbo].[taUpdateCreateCustomerRcdPost]  @I_vCUSTNMBR char(15),    @I_vHOLD tinyint,    @I_vINACTIVE tinyint,    
...
IF NOT EXISTS(SELECT CUSTVNDR FROM clLC0002 WHERE CUSTVNDR = @I_vCUSTNMBR AND TipClieProvee = 1)
BEGIN
	INSERT INTO clLC0002 (custvndr, rutClieProvee, tipClieProvee, 
						GiroEmpresa, RazonSocial, RutRepLegal, NomRepLeg, Activo)
				SELECT @I_vCUSTNMBR CUSTVNDR, upper(LEFT(replace(replace(@I_vTXRGNNUM, '.', ''), '-', ''), 11)) RutClieProvee, 1 TipClieProvee, 
					LEFT(@I_vCOMMENT1, 41) GiroEmpresa, LEFT(@I_vCUSTNAME, 61) RazonSocial, '' RutRepLegal, '' NomRepLeg, 1 Activo
END
ELSE
BEGIN
	UPDATE clLC0002 SET RutClieProvee = upper(LEFT(replace(replace(@I_vTXRGNNUM, '.', ''), '-', ''), 11)), 
						GiroEmpresa = LEFT(@I_vCOMMENT1, 41), 
						RazonSocial = LEFT(@I_vCUSTNAME, 61)
	WHERE CUSTVNDR = @I_vCUSTNMBR AND TipClieProvee = 1
END
